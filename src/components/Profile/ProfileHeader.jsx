...
const ProfileHeader = () => {
    ...
    const {handlefollowUser, isfollowing , isuploading} = useFollowerUser(userProfile.uid);
    if(isOpen){
        return <EditProfile onOpen={onOpen}  isOpen={isOpen} onClose={onClose}/>
    }
    return (
        <Flex gap={{ base: 4, sm: 10 }} py={10} direction={{ base: "column", sm: "row" }}>
            <AvatarGroup size={{ base: "xl", md: "2xl" }} justifySelf={"center"} alignSelf={"flex-start"} mx={"auto"}>
                <Avatar name={userProfile.fullName} src={userProfile.profilePicURL} alt={userProfile.fullName + " logo"} />
            </AvatarGroup>
            <VStack alignItems={"start"} gap={2} mx={"auto"} flex={1}>
                <Flex gap={4} direction={{ base: "column", sm: "row" }} justifyContent={{ base: "center", sm: "flex-start" }} alignItems={"center"} w={"full"}>
                    ...
                    {visitingAnotherProfileAndAuth && (
                        <Flex gap={4} alignItems={"center"} justifyContent={"center"}>
                        <Button onClick={handlefollowUser} isLoading={isuploading} bg={"blue.500"} color={"white"} _hover={{ bg: "blue.600" }} size={{ base: "xs", md: "sm" }}>
